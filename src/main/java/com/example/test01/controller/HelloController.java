...
@Controller
//분기점. @controller 선언을 해주면 Spring boot는 HelloController 클래스가 컨트롤 담당이라고 인식
...
public class HelloController {
    ...
    public String hello(Model model) {
        ...
        //Controller는 return을 통해 '알아서' hello라는 이름의 html 파일을 찾는다.
        return "hello";
    }

    //호출하는 클라이언트의 정보를 가져다가 서버(controller)에 전달해주는 매핑
    //클라이언트가 요정한 정보를 가져오는 어노테이션

    //중괄호{}안에 데이터를 컨트롤러에 전달 = url이라는 변수에 데이터를 전달
    //@RequestMapping 어노테이션이 url 정보를 갖고 있기 때문에
    ...
    public String account(@RequestParam("id1") String id, Model model) {
        ...
        //Staric으로 선언한 문자열 변수를 return하여 String 메모리를 절약
        return account;
